...
void MPU9250::writeByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
  ...
  i2c_t3(I2CPort).write(data);                 // Put data in Tx buffer
  i2c_t3(I2CPort).endTransmission();           // Send the Tx buffer
}
...
uint8_t MPU9250::readByte( uint8_t address, uint8_t subAddress)
{
  ...
  i2c_t3(I2CPort).beginTransmission(address);         // Initialize the Tx buffer
  i2c_t3(I2CPort).write(subAddress);                  // Put slave register address in Tx buffer
  i2c_t3(I2CPort).endTransmission(false);             // Send the Tx buffer, but send a restart to keep connection alive
  i2c_t3(I2CPort).requestFrom(address, (uint8_t) 1);  // Read one byte from slave register address 
  data = i2c_t3(I2CPort).read();                      // Fill Rx buffer with result
...
{  
    ...
    i2c_t3(I2CPort).endTransmission(false);       // Send the Tx buffer, but send a restart to keep connection alive
  uint8_t i = 0;
  i2c_t3(I2CPort).requestFrom(address, count);  // Read bytes from slave register address 
